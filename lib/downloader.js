...
export async function resolveSpotifyUrl(spotifyUrl) {
    if (extractPlaylistId(spotifyUrl)) return { kind: 'playlist', ...(await getSpotifyPlaylist(spotifyUrl)) };
    if (extractAlbumId(spotifyUrl)) return { kind: 'album', ...(await getSpotifyAlbum(spotifyUrl)) };
    if (extractTrackId(spotifyUrl)) {
        const track = await getSpotifySong(spotifyUrl);
        return { kind: 'track', title: track.name, owner: track.artist, art: track.art, tracks: [track] };
    }
    throw new Error('That does not look like a Spotify track, album or playlist link.');
}
